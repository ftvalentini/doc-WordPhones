...
NOTE for spanish, previously download from: https://corpus.rae.es/lfrecuencias.html
"""
...
class Downloader:
    ...
    def run(self):
        ...
        try:
            words = get_search_words(self.session, self.config.lang, self.config.timeout)
        ...
        if self.config.random:
            logger.info("Shuffling words for random processing.")
...
def get_search_words(session: requests.Session, lang: str, timeout: int) -> List[str]:
    ...
    if lang == "es":
        ...
        # read data/CREA_total.TXT:
        df_words = pd.read_csv(
            "data/CREA_total.TXT",
            sep="\t",
            encoding="Latin-1",
            names=["rank", "words", "freq_abs", "freq_rel"],
            header=0,
        )
        # Keep max 300k:
        words = df_words["words"].str.strip().tolist()[:300_000]
    return words
...
if __name__ == "__main__":
    ...
    parser.add_argument("--random", action="store_true", help="Search words in random order.")
    args = parser.parse_args()
    config = Config(outdir=args.outdir, lang=args.lang, random=args.random)
